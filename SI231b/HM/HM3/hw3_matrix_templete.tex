...
\begin{enumerate}
	\item Proof:
	suppose a basis of $\mathcal{U}$ is $\vec{v}_{1}, \vec{v}_{2}, \ldots, \vec{v}_{m}$.and we can turn the basis $\vec{v}_{i}$ into an orthonormal basis $\vec{u}_{i},$ using the Gram-Schmidt algorithm.
	Then orthogonal projector will be: $P=\sum \vec{u}_{i} \vec{u}_{i}^{T}$,Note that this is an $m \times m$ matrix.
	
	\item
	Consider a subspace $\mathcal{U}$ of $\mathbb{R}^{n}$ with basis $\vec{v}_{1}, \vec{v}_{2}, \ldots, \vec{v}_{m} .$ Let
	$$
	...
	$$
	We are not required to find an orthonormal basis of $V$ here. If the vectors $\vec{v}_{1} \ldots . . \vec{v}_{m}$ happen to be orthonormal, then $A^{T} A=I_{m}$ and the formula simplifies to $A A^{T} .
